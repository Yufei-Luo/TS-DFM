...
from reactot.analyze.rmsd import batch_rmsd_sb, batch_rmsd, batch_dmae_sb
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit":
            ...
            self.val_dataset = func(
                Path(self.training_config["datadir"], f"valid{ft}"),
                **self.training_config,
            )
        elif stage == "test":
        ...
        elif stage == "test_ood_size":
            self.test_dataset = func(
                Path(self.training_config["datadir"], f"test_ood_size{ft}"),
                **self.training_config,
            )

        elif stage == "test_ood_type":
            self.test_dataset = func(
                Path(self.training_config["datadir"], f"test_ood_type{ft}"),
                **self.training_config,
            )
        else:
...
class SBModule(LightningModule):
    ...
    def compute_loss(self, batch, test=False):
        if test:
            representations, conditions, _ = batch
        else:
            representations, conditions = batch
        loss_terms = self.ddpm.forward(
            representations,
    ...
    def eval_sample_batch(
        ...
        self.ddpm.eval()
        if test:
            representations, conditions, ediff = batch
        else:
            representations, conditions = batch
        ...
        with torch.no_grad():
            ...
            info = self.compute_loss(batch, test)
        x0_pred = xs[:, 0, ...]
        ...
        dmaes = batch_dmae_sb(x0_size.cpu(), pred_xh, target_xh, same_order=True)

        if return_all:
            return cond["r_pos"], x0_pred.cpu(), cond["p_pos"], x0_size, x0_other, rmsds
        
        if test:
            return x0_pred.cpu(), representations, conditions, ediff, rmsds, dmaes
    ...
    def validation_step(self, batch, batch_idx, *args):
        temp = self._shared_eval(batch, batch_idx, "val", *args)
        self.validation_step_outputs.append(temp)
        return temp

    def test_step(self, batch, batch_idx, *args):
        temp = self._shared_eval(batch, batch_idx, "test", *args)
        self.test_step_outputs.append(temp)
        return temp

    def on_validation_epoch_end(self):
        val_epoch_metrics = average_over_batch_metrics(self.validation_step_outputs)
        if self.trainer.is_global_zero:
            pretty_print(self.current_epoch, val_epoch_metrics, prefix="val")
        val_epoch_metrics.update({"epoch": self.current_epoch})
        for k, v in val_epoch_metrics.items():
    ...
    def on_train_epoch_end(self) -> None:
        epoch_metrics = average_over_batch_metrics(self.training_step_outputs, allowed=self.eval_keys)
