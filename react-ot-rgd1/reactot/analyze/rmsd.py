...
def dmae_core(mol1, mol2, threshold=0.5, same_order=False):
    ...
    if same_order:
        ...
        loss = np.mean(np.abs(dists1 - dists2))
        return loss
...
def pymatgen_dmae(
    ...
    if isinstance(mol2, str):
        mol2 = xyz2pmg(mol2)
    dmae = dmae_core(mol1, mol2, threshold, same_order)
    if ignore_chirality:
        ...
        dmae_reflect = dmae_core(mol1, mol2_reflect, threshold, same_order)
        dmae = min(dmae, dmae_reflect)
    return dmae
