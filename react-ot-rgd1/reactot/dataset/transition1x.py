...
class ProcessedTS1x(BaseDataset):
    def __init__(
        ...
        **kwargs,
        ...
        # single_frag_inds = np.array(range(len(self.raw_dataset["reactant"]['rxn'])))
        # use_inds = range(len(self.raw_dataset["reactant"]['rxn']))
        ...
        if ediff is not None:
            self.data["ediff"] = torch.tensor([temp.item() for temp in self.raw_dataset[ediff]["ediff"]]).unsqueeze(1)
        # if ts_guess:
