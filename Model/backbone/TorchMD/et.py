...
class TorchMD_ET(nn.Module):
    ...
    def __init__(
        ...
        self.distance_expansion = rbf_class_mapping[rbf_type](
            self.cutoff_lower, self.cutoff_upper, self.num_rbf, self.trainable_rbf
        )
        self.neighbor_embedding = (
            NeighborEmbedding(
                self.hidden_channels, self.num_rbf + self.num_edges, self.cutoff_lower, self.cutoff_upper, self.max_z
            ).jittable()
        ...
        if out_norm:
            self.out_norm = nn.LayerNorm(self.hidden_channels)
        else:
            self.out_norm = None
    ...
    def forward(self, z, pos, edge_index, edge_attr, batch):
        ...
        rbf_attr = self.distance_expansion(edge_weight)
        if edge_attr is not None:
            edge_attr = torch.concat([edge_attr, rbf_attr], dim=1)
        else:
            edge_attr = rbf_attr
