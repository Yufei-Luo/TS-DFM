...
try:
    from rdkit.Chem import rdEHTTools, rdMolDescriptors #requires RDKit 2019.9.1 or later
except ImportError:
    rdEHTTools = None
...
def visualize_mol_pos(x, pos, size=(300, 300)) -> py3Dmol.view:
    natoms = len(x)
    mol = f"{natoms}\n\n"
...
def draw_molecule_list(x, pos_list, width=1000, height=300) -> py3Dmol.view:
    natoms = len(x) * len(pos_list)
    mol = f"{natoms}\n\n"
    for ii, pos in enumerate(pos_list):
        pmatg_mol_prime = Molecule(
            species=x,
            coords= pos + np.array([15.0, 0.0, 0.0]) * ii,
        )
        mol += "\n".join(pmatg_mol_prime.to(fmt="xyz").split("\n")[2:]) + "\n"
    viewer = py3Dmol.view(height, width)
    viewer.addModel(mol, "xyz")
    viewer.setStyle({'stick': {}, "sphere": {"radius": 0.3}})
    viewer.zoomTo()
    return viewer

def transfrom_to_rdmol(atomic_numbers, positions):
    if not isinstance(atomic_numbers, np.ndarray):
        atomic_numbers = atomic_numbers.cpu().numpy()
    if not isinstance(positions, np.ndarray):
        positions = positions.cpu().numpy()
    bond, _ = xyz2AC(atomic_numbers.tolist(), positions.tolist(), 0)
    try:
        mol = xyz2mol(atomic_numbers.tolist(), positions.tolist())
    except:
        mol = None
    return bond, mol

class ReactionFeatureExtracter:
    def __init__(self):
        pass
    
    def count_molecule_fragment(self, adj_matrix):
        n = len(adj_matrix)
        visited = [False] * n
        count = 0
    ...
    def extract_reaction_barrier(self, reactant_energy, transition_state_energy):
        reaction_barrier = transition_state_energy - reactant_energy
        return reaction_barrier
    
    def extract_bond_change_cnt(self, reactant_bond, product_bond):
        bond_breaking_cnt = np.sum((reactant_bond == 1) & (product_bond == 0))
        bond_formation_cnt = np.sum((reactant_bond == 0) & (product_bond == 1))
        return int(bond_breaking_cnt) // 2, int(bond_formation_cnt) // 2
    
    def extract_react_centers(self, reactant_bond, product_bond):
        bond_diff = (reactant_bond != product_bond).astype(np.int32)
        n_atoms = reactant_bond.shape[0]
    ...
    def is_react_on_rings(self, mol_reactant, react_centers):
        ...
        for ring in atom_rings:
            is_aromatic_ring = all(idx in aromatic_atoms for idx in ring)
            
            for atom_idx in ring:
                atoms_on_rings.add(atom_idx)
                if is_aromatic_ring:
                    aromatic_ring_atoms.add(atom_idx)
                else:
                    non_aromatic_ring_atoms.add(atom_idx)
        
        react_centers_on_rings = react_centers.intersection(atoms_on_rings)
        
        if len(react_centers_on_rings) == 0:
            return 0
        
        if len(react_centers.intersection(aromatic_ring_atoms)) > 0:
            return 1
        
        if len(react_centers.intersection(non_aromatic_ring_atoms)) > 0:
            return 2
        
        return 0
    
    def ts_volume(self, pos_ts):
        from scipy.spatial import ConvexHull
        hull = ConvexHull(pos_ts)
        return hull.volume
    
    def extract_rotatable_bond_cnt(self, mol_reactant):
        num_rotatable = rdMolDescriptors.CalcNumRotatableBonds(
            mol_reactant, 
            strict=True
        )
        return num_rotatable
    
    def extract_molecular_features(self, x, reactant_pos, product_pos, transition_state_pos, reactant_energy, ts_energy):
        reactant_bond, mol_reactant = transfrom_to_rdmol(x, reactant_pos)
        product_bond, mol_product = transfrom_to_rdmol(x, product_pos)
        ts_bond, mol_ts = transfrom_to_rdmol(x, transition_state_pos)

        if not isinstance(x, np.ndarray):
            x = x.cpu().numpy()
        if not isinstance(reactant_pos, np.ndarray):
            reactant_pos = reactant_pos.cpu().numpy()
        if not isinstance(product_pos, np.ndarray):
            product_pos = product_pos.cpu().numpy()
        if not isinstance(transition_state_pos, np.ndarray):
            transition_state_pos = transition_state_pos.cpu().numpy()

        mol_reactant = mol_reactant[0] if (mol_reactant is not None and len(mol_reactant) > 0) else None
        mol_product = mol_product[0] if (mol_product is not None and len(mol_product) > 0) else None

        barrier = self.extract_reaction_barrier(reactant_energy, ts_energy)
        bond_breaking_cnt, bond_formation_cnt = self.extract_bond_change_cnt(reactant_bond, product_bond)
        react_centers = self.extract_react_centers(reactant_bond, product_bond)
        is_react_on_rings = self.is_react_on_rings(mol_reactant, react_centers) if mol_reactant is not None else -1
        ts_volume = self.ts_volume(transition_state_pos)
        rotatable_bond_cnt = self.extract_rotatable_bond_cnt(mol_reactant) if mol_reactant is not None else -1
        n_fragment = self.count_molecule_fragment(ts_bond)
        features = {
            'bond_breaking_cnt': bond_breaking_cnt,
            'bond_formation_cnt': bond_formation_cnt,
            'reaction_barrier': barrier,
            'reactant_center_cnt': len(react_centers),
            'is_react_on_rings': is_react_on_rings,
            'ts_volume': ts_volume,
            'rotatable_bond_cnt': rotatable_bond_cnt,
            'ts_fragment_cnt': n_fragment
        }
        return features